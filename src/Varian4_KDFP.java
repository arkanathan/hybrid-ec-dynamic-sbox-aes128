...
// Varian 4: KDFP (Key-Derived Final Parameterization)
// Konstruksi affine-equivalent; parameter output (A_k, c_k) diturunkan dari h1
...
public class Varian4_KDFP {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("🚀 VARIAN 4: EC-Keyed Affine-Equivalent Final Layer (KDFP Revised)");
        System.out.println("S(x) = A_k(Inv(B_k·x ⊕ b_k)) ⊕ c_k | Output affine (A,c) driven by h1 / R-points");
        ...
        System.out.println("\n" + "=".repeat(130));
        System.out.println("=== VARIAN 4 AVERAGE (" + RUNS + " Runs) ===");
    ...
    private static int[] constructSBox(String sessionKey) throws Exception {
        ...
        for (int i = 0; i < NUM_D_CANDIDATES; i++) {
            ...
            int[] sbox = buildOutputKeyedAffineEquivalentSBox(sessionKey, h0, h1, i);
    ...
    // KDFP: h1 mengekang parameter affine sisi keluaran (A_k, c_k).
    private static int[] buildOutputKeyedAffineEquivalentSBox(String sessionKey,
                                                              List<Integer> h0,
                                                              List<Integer> h1,
                                                              int dIndex) throws Exception {
        byte[] seed = deriveSeed(sessionKey, "v4output", h0, h1, dIndex);

        int[] inputMatrix = deriveInvertibleMatrix(seed, 0);
        int inputConstant = seed[16] & 0xFF;

        byte[] outputSeed = MessageDigest.getInstance("SHA-256").digest(
                concatBytes(seed, materialFingerprint(h1, 96).getBytes(StandardCharsets.UTF_8))
        );
        int[] outputMatrix = deriveInvertibleMatrix(outputSeed, 0);
        int outputConstant = (outputSeed[20] ^ mixList(h1)) & 0xFF;
    ...
    private static byte[] deriveSeed(String sessionKey,
                                     ...
                                     List<Integer> h1,
                                     int dIndex) throws Exception {
        String payload = sessionKey + label + dIndex + materialFingerprint(h0, 64) + materialFingerprint(h1, 64);
    ...
    private static int mixList(List<Integer> material) {
        int mix = 0;
        int count = Math.min(64, material.size());
        for (int i = 0; i < count; i++) {
            mix ^= material.get(i);
            mix = Integer.rotateLeft(mix, 3);
        }
        return mix & 0xFF;
    }
