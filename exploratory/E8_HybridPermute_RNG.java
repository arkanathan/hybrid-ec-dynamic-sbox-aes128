// Exploratory / pra-final design — label E8 (Lampiran A).
// Bukan rancangan official Bab 4. Official: src/Varian1..Varian4_*.java
// Legacy source class name: E8_HybridPermute_RNG
...
public class E8_HybridPermute_RNG {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("🚀 VARIAN 4B: Stage 2 – RNG-Injected Permutation-Code Hybrid");
        ...
        for (int run = 0; run < runs; run++) {
            ...
            List<Integer> h0_intermediate = keyedByteMixing(Q_points, k, true);   // x-dominant
            List<Integer> h1_intermediate = keyedByteMixing(R_points, k, false);  // y-dominant

            // === STAGE 2 (Varian 4B): RNG-Injected Permutation-Code ===
            List<Integer> rngStreamH0 = generateKeyedRNGStream(k, "H0", h0_intermediate.size());
            List<Integer> rngStreamH1 = generateKeyedRNGStream(k, "H1", h1_intermediate.size());

            List<Integer> h0_final = buildHybridMaterial(h0_intermediate, rngStreamH0);
            List<Integer> h1_final = buildHybridMaterial(h1_intermediate, rngStreamH1);

            // Two-stage Fisher-Yates menggunakan material final
            int[] S0 = new int[256];
            for (int i = 0; i < 256; i++) S0[i] = i;

            int[] S1 = fisherYatesShuffle(S0.clone(), h0_final);
            int[] S2 = fisherYatesShuffle(S1, h1_final);
        ...
        System.out.println("\n" + "=".repeat(95));
        System.out.println("=== VARIAN 4B AVERAGE - Stage 2 RNG-Injected Permutation-Code Hybrid ===");
    ...
    // STAGE 2 (Varian 4B): RNG stream dengan domain separation
    private static List<Integer> generateKeyedRNGStream(BigInteger k, String domain, int requiredLength) throws Exception {
    ...
    // Hybrid injection ke material intermediate Stage 1
    private static List<Integer> buildHybridMaterial(List<Integer> intermediate, List<Integer> rngStream) {
        List<Integer> finalMaterial = new ArrayList<>();
        for (int i = 0; i < intermediate.size(); i++) {
            int ecByte = intermediate.get(i);
            int rngByte = rngStream.get(i);
            int m = (ecByte ^ rngByte) & 0xFF;
            m = Integer.rotateLeft(m, (rngByte & 0x07)) & 0xFF;
            m = (m + (i % 256)) & 0xFF;
            finalMaterial.add(m);
        }
