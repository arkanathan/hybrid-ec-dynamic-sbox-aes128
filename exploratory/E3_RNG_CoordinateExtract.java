// Exploratory / pra-final design — label E3 (Lampiran A).
// Bukan rancangan official Bab 4. Official: src/Varian1..Varian4_*.java
// Legacy source class name: E3_RNG_CoordinateExtract
...
public class E3_RNG_CoordinateExtract {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("🚀 VARIAN 3 New – RNG-Controlled Coordinate Extraction Hybrid");
        ...
        System.out.println("\n" + "=".repeat(95));
        System.out.println("=== VARIAN 3 New AVERAGE - RNG-Controlled Coordinate Extraction Hybrid ===");
    ...
    // === FUNGSI INTERLEAVE YANG SUDAH DIPERBAIKI (12 byte eksplisit) ===
    private static List<Integer> buildControlledMaterial(List<ECPoint> points, List<Integer> rngStream, boolean isH0) {
        ...
        for (ECPoint pt : points) {
            ...
            // INTERLEAVE 12 BYTE – EXPLICIT LAYOUT
            byte[] result = new byte[12];
            if (interleaveMode == 0) {           // d0 s0 d1 s1 d2 s2 d3 s3 d4 d5 d6 d7
                for (int i = 0; i < 4; i++) {
                    result[2 * i] = domBytes[i];
                    result[2 * i + 1] = supBytes[i];
                }
                System.arraycopy(domBytes, 4, result, 8, 4);
            } else if (interleaveMode == 1) {    // s0 d0 s1 d1 s2 d2 s3 d3 d4 d5 d6 d7
                for (int i = 0; i < 4; i++) {
                    result[2 * i] = supBytes[i];
                    result[2 * i + 1] = domBytes[i];
                }
                System.arraycopy(domBytes, 4, result, 8, 4);
            } else if (interleaveMode == 2) {    // d0 d1 s0 s1 d2 d3 s2 s3 d4 d5 d6 d7
                ...
                System.arraycopy(domBytes, 4, result, 8, 4);
            } else {                             // s0 s1 d0 d1 s2 s3 d2 d3 d4 d5 d6 d7
    ...
    // ==================== METRIK & SHUFFLE (sama seperti sebelumnya) ====================
