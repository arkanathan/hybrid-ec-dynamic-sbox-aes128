// Exploratory / pra-final design — label E5 (Lampiran A).
// Bukan rancangan official Bab 4. Official: src/Varian1..Varian4_*.java
// Legacy source class name: E5_InterleaveMaterial
...
public class E5_InterleaveMaterial {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("🚀 VARIAN 3_New_v3A1 – Expanded Interleave Family (Ratio Fixed 8:4)");
        ...
        for (int run = 0; run < runs; run++) {
            ...
            List<Integer> rngStreamH0 = generateRNGStream(k, "EXT_H0", Q_points.size() * 4); // +1 untuk mode
            List<Integer> rngStreamH1 = generateRNGStream(k, "EXT_H1", R_points.size() * 4);

            List<Integer> h0 = buildControlledMaterial(Q_points, rngStreamH0, true);
            List<Integer> h1 = buildControlledMaterial(R_points, rngStreamH1, false);
        ...
        System.out.println("\n" + "=".repeat(95));
        System.out.println("=== VARIAN 3_New_v3A1 AVERAGE - Expanded Interleave Family (Ratio Fixed 8:4) ===");
    ...
    // ==================== HELPER METHODS (identik dengan Varian3_New) ====================
    ...
    private static List<Integer> buildControlledMaterial(List<ECPoint> points, List<Integer> rngStream, boolean isH0) {
        ...
        for (ECPoint pt : points) {
            ...
            int interleaveMode = rngStream.get(rngIdx++) % 6;   // 0-5 sesuai 6 mode yang dikunci
            ...
            switch (interleaveMode) {
                case 0: // Mode 0
                    for (int i = 0; i < 4; i++) { result[2*i] = domBytes[i]; result[2*i+1] = supBytes[i]; }
                    System.arraycopy(domBytes, 4, result, 8, 4);
                    break;
                case 1: // Mode 1
                    for (int i = 0; i < 4; i++) { result[2*i] = supBytes[i]; result[2*i+1] = domBytes[i]; }
                    System.arraycopy(domBytes, 4, result, 8, 4);
                    break;
                case 2: // Mode 2
                    System.arraycopy(domBytes, 0, result, 0, 8);
                    System.arraycopy(supBytes, 0, result, 8, 4);
                    break;
                case 3: // Mode 3
                    System.arraycopy(supBytes, 0, result, 0, 4);
                    System.arraycopy(domBytes, 0, result, 4, 8);
                    break;
                case 4: // Mode 4
                    result[0] = domBytes[0]; result[1] = domBytes[1];
                    result[2] = supBytes[0]; result[3] = supBytes[1];
                    result[4] = domBytes[2]; result[5] = domBytes[3];
                    result[6] = supBytes[2]; result[7] = supBytes[3];
                    result[8] = domBytes[4]; result[9] = domBytes[5];
                    result[10] = domBytes[6]; result[11] = domBytes[7];
                    break;
                case 5: // Mode 5
                    result[0] = domBytes[0]; result[1] = domBytes[1]; result[2] = domBytes[2];
                    result[3] = supBytes[0]; result[4] = supBytes[1]; result[5] = supBytes[2]; result[6] = supBytes[3];
                    result[7] = domBytes[3]; result[8] = domBytes[4]; result[9] = domBytes[5];
                    result[10] = domBytes[6]; result[11] = domBytes[7];
                    break;
            }
