// Exploratory / pra-final design — label E7 (Lampiran A).
// Bukan rancangan official Bab 4. Official: src/Varian1..Varian4_*.java
// Legacy source class name: E7_FinalPermute_Revised
...
public class E7_FinalPermute_Revised {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("🚀 VARIAN 4: Stage 2 – Keyed RNG-Driven Final Permutation (Revised Final)");
        ...
        for (int run = 0; run < runs; run++) {
            ...
            // === STAGE 2 REVISED: Keyed RNG-Driven Final Permutation ===
        ...
        System.out.println("\n" + "=".repeat(95));
        System.out.println("=== VARIAN 4 AVERAGE - Stage 2 Keyed RNG-Driven Final Permutation (Revised Final) ===");
    ...
    // STAGE 2 REVISED: Keyed RNG-Driven Final Permutation dengan repeated SHA-256 expansion
    private static int[] finalKeyedPermutation(int[] sbox, BigInteger k) throws Exception {
        MessageDigest md = MessageDigest.getInstance("SHA-256");
        byte[] seed = md.digest(k.toByteArray());           // SHA-256 hanya untuk seed derivation

        // Deterministic keyed random byte stream via repeated SHA-256 + counter
        List<Integer> randomStream = new ArrayList<>();
        byte[] current = seed;
        long counter = 0;
        while (randomStream.size() < 4096) {                // cukup untuk Fisher-Yates 256 elemen
            byte[] input = new byte[current.length + 8];
            System.arraycopy(current, 0, input, 0, current.length);
            for (int i = 0; i < 8; i++) {
                input[current.length + i] = (byte) (counter >>> (56 - i * 8));
            }
            current = md.digest(input);
            for (byte b : current) {
                randomStream.add((int) b & 0xFF);
                if (randomStream.size() >= 4096) break;
            }
            counter++;
        }

        // Final Fisher-Yates menggunakan randomStream
        int[] s = sbox.clone();
        for (int i = s.length - 1; i > 0; i--) {
            int j = randomStream.get(i) % (i + 1);
            int temp = s[i];
            s[i] = s[j];
            s[j] = temp;
        }
